...
Let us formalize the notion of differential privacy, following the introduction
in \cite{dwork2014algorithmic}.

Generally, we consider datasets that are collections of records from some
...
queries. To preserve privacy, we say that we want the response of this mechanism
...
such that $\norm{x - y}_1 \leq 1$,
\begin{align*}
P(\mathcal{M}(x) \in \mathcal{S}) \leq \exp{(e)} P(\mathcal{M}(y) \in \mathcal{S}) + \delta
\end{align*}
\end{definition}

There are a number of basic mechanisms for ensuring differential privacy. A
simple, intuitive way of ensuring differential privacy is to add noise to the
...
The exponential mechanism $\Mc_E(x, u, \Rc)$ outputs an elmeent $r \in \Rc$ with
