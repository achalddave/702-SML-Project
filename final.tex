...
\begin{algorithm}[htb]
\begin{algorithmic}[1]
  \Function{Output Perturbation}{dataset $\Dc$, differential privacy $\epsilon$, regularization $\Lambda$}
...
\begin{theorem} Let $f_0$ be any classifier. Let $L(f) = \Eb_{x, y}\; [l(f^T x, y)]$ be the true loss. Let $l$ be differentiable and continuous, and let $l'$ be $c$-Lipschitz with $\abs{l'} \le 1$. Then, there exists a constant $C$ s.t. when the number of training samples $n$ satisfies
